...
const DataGridE = () => {
  ...
  const columns = [
    ...
    {
      field: "total_reservado",
      headerName: "Cantidad Reservada",
      type: "number",
      flex: 1,
    },
    {
      field: "total_por_recibir",
      headerName: "Cantidad Por Ingresar",
      type: "number",
      flex: 1,
    },
    {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter((exist) => {
        ...
        const productInventoryId = exist.inventory_id
          ? exist.inventory_id.toLowerCase()
          : "";
        const productVariationDesc = exist.variation_desc
          ? exist.variation_desc.toLowerCase()
          : "";
        const productoLocalidad = exist.localidad_descripcion
          ? exist.localidad_descripcion.toLowerCase()
          : "";

        // Convertir la cantidad a cadena y hacer la búsqueda
        const productoCantidad =
          exist.cantidad !== undefined
            ? exist.cantidad.toString().toLowerCase()
            : "";
